...
def build_dataset(tokenizer, texts: List[str], block_size: int, is_causal: bool):
    ...
    if len(ids) == 0:
        raise RuntimeError("No valid tokenized sequences found")
    
    all_ids = torch.cat(ids)
    n_blocks = all_ids.size(0) // block_size
    
    if n_blocks == 0:
        raise RuntimeError(f"Not enough tokens to create even one block of size {block_size}")
    
    all_ids = all_ids[: n_blocks * block_size]
    all_ids = all_ids.view(n_blocks, block_size)

    # Create dataset from dictionary instead of generator to avoid filesystem issues
    data_dict = {"input_ids": [all_ids[i].tolist() for i in range(all_ids.size(0))]}
...
def group_targets_for_peft(model, model_name: str, schedule_df: Optional[pd.DataFrame]):
    ...
    if schedule_df is not None and "feature" in schedule_df.columns:
        df = schedule_df.copy()
