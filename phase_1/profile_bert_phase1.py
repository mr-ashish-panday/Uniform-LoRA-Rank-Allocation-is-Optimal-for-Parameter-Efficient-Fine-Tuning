#!/usr/bin/env python3
"""
phase1_metaloRAMem.py - Modified for GPT2-medium profiling
...
MODIFICATION: Changed to GPT2LMHeadModel for full transformer profiling
"""
...
from transformers import (
    GPT2LMHeadModel,  # CHANGED: from AutoModelForSequenceClassification
    AutoTokenizer,
...
def train_step(model, batch, optimizer, device, scheduler=None, grad_accum_steps=1):
    ...
    # CHANGED: GPT2 uses labels = input_ids for language modeling
    labels = inputs['input_ids'] if 'input_ids' in inputs else None
...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", type=str, default="gpt2-medium")  # CHANGED: default to gpt2-medium
...
def main():
    ...
    # CHANGED: Use GPT2LMHeadModel instead of AutoModelForSequenceClassification
    ...
    print(f"Found {original_linear_count} nn.Linear modules.")
    ...
    activations_csv = args.log_csv.replace(".csv", "_activations3.csv")
    ...
    gradnorms_csv = args.log_csv.replace(".csv", "_gradnorms3.csv")
    ...
    if args.mode == "train":
        ...
        for epoch in range(args.epochs):
            ...
            # CHANGED: Skip validation accuracy computation for GPT2 (language modeling task)
            print(f"Epoch {epoch+1} training complete. Mean loss: {np.mean(losses):.4f}")
